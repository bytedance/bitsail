...
import com.bytedance.bitsail.connector.larksheet.api.response.OpenApiBaseResponse;
import com.bytedance.bitsail.connector.larksheet.api.response.SheetMetaInfoResponse;
import com.bytedance.bitsail.connector.larksheet.api.response.SheetRangeResponse;
...
import static com.bytedance.bitsail.connector.larksheet.constant.LarkSheetConstant.SPLIT_COLON;
...
public class LarkSheetUtil {
  ...
  /**
   * Transform an integer into character sequence.<br/>
   * 1 --> A, 2 --> B, 26 --> Z, 27 --> AA.
   *
  ...
  public static List<SheetInfo> resolveSheetUrls(List<String> sheetUrls) throws BitSailException {
    List<Pair<String, String>> sheetTokenAndIds = new ArrayList<>();
   ...
   */
  public static List<SheetInfo> transformSheetInfo(final List<Pair<String, String>> sheetTokenAndIds) {
    List<SheetInfo> sheetInfoList = new ArrayList<>();
    for (Pair<String, String> sheetTokenAndId : sheetTokenAndIds) {
      String sheetToken = sheetTokenAndId.getFirst();
      String sheetId = sheetTokenAndId.getSecond();
  ...
  /**
   * Get metadata of a sheet (mainly include row number and column number).<br/>
   ...
   */
  public static Map<String, String> fillLarkParams(Map<String, String> features) {
    Map<String, String> rangeParamMap;
    if (MapUtils.isEmpty(features)) {
      rangeParamMap = Maps.newHashMap();
    } else {
      rangeParamMap = features;
    }
    for (Feature feature : Feature.values()) {
      rangeParamMap.putIfAbsent(feature.key, feature.value);
    }
    return rangeParamMap;
  }
   ...
   */
  public static String genSheetRange(SheetHeader sheetHeader, int startRowNumber, int endRowNumber, SheetMeta sheetMeta) {
    return sheetHeader.getStartColumn() +
        startRowNumber +
        SPLIT_COLON +
        sheetHeader.getEndColumn() +
