...
public class HashReaderSelectorTest {
  ...
  private final String indexPrefix = "test_index_";

  @Before
  public void setup() {
    selector = new HashReaderSelector(readerNumber);
  }
  ...
  public void testGetReaderIndex() {
    ...
    for (int i = 0; i < splitNumber; i++) {
      int idx = selector.getReaderIndex(indexPrefix + i);
      List<Integer> list = map.getOrDefault(idx, Lists.newArrayList());
      list.add(i);
      map.put(idx, list);
    }
    assertEquals(3, map.get(0).size());
