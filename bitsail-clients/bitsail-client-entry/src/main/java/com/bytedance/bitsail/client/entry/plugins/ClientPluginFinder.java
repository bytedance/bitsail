...
public class ClientPluginFinder extends LocalFSPluginFinder {
  ...
  protected List<PluginStore> createPluginStores(BitSailConfiguration commonConfiguration) {
    ...
    Path frameworkClientsDirPath = PackageResolver.getClientDir(frameworkBaseDir);

    pluginStores.add(PluginStore.builder()
        .pluginBaseDirPath(frameworkClientsDirPath.resolve(pluginDirName))
        .pluginMappingBaseDirPath(frameworkClientsDirPath.resolve(pluginMappingDirName))
        .build());
